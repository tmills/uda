...
class PivotLearnerModel(nn.Module):
    ...
    def __init__(self, input_features):
        ...
        # Replace with a layer that has all 0's off the diagonal
        self.feature.add_module('input_layer', nn.Linear(input_features, input_features))

        # task_classifier maps from a feature representation to a task prediction
        self.task_classifier = nn.Sequential()
        self.task_classifier.add_module('task_binary', nn.Linear(input_features, 1))
        ...
        self.domain_classifier.add_module('domain_linear', nn.Linear(input_features, 1))
...
def main(args):
    ...
    for epoch in range(epochs):
        ...
        for ind in range(num_train_instances):
            ...
            weights = model.feature.weights
            hollow_weights = weights - torch.diag(torch.diag(weights))

            off_diag_weight = hollow_weights.sum()
            total_loss += off_diag_weight
        ...
        weights = torch.diag(model.feature.weights)
